...
DOCUMENTATION = """
    ...
    description:
      ...
      - Logging stdout straight to a file.
    options:
      nocolor:
        default: False
...
class CallbackModule(CallbackBase):
    ...
    def _print_log(self, changed, msg, diff, is_host, error, stdout, stderr):
        if is_host:
            indent_level = 0
            #name = colorize(host_or_item.name, 'not_so_bold')
            name = ""
    ...
    def _print_task_result(self, result, error=False, **kwargs):
        ...
        if 'print_action' in result._task.tags or error or self._display.verbosity > 1:
            ...
            self._print_log( result._result.get('changed', False),
                             msg,
            ...
            if 'results' in result._result:
                for r in result._result['results']:
                    ...
                    self._print_log( r.get('changed', False),
                                             to_text(r.get('msg', '')),
                                             r.get('diff', None),
                                             is_host=False,
                                             error=failed,
    ...
    def v2_playbook_on_stats(self, stats):
        ...
        for host in hosts:
            ...
            msg = '{0}    : failed={1}\tunreachable={2}'.format(
                host, s['failures'], s['unreachable'])
